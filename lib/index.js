 ...
 * @Last Modified time: 2024-08-06 17:20:02
...
function getAppEntry(configFile) {
  ...
  let appPathUrl = configFile
    .replace(extname(configFile), "")
    .replace(new RegExp(`(\.${config.platform})$`), "");
  if (config.fileSource) {
    appPathUrl = appPathUrl.replace(new RegExp(`(\.${config.fileSource})$`), "");
  }
  ...
  entryName = config.fileSource
    ? entryName.replace(new RegExp(`(\.${config.fileSource})$`), "")
    : entryName;
...
function parsePages(pages) {
  ...
  pages.forEach((page) => {
    ...
    const pageJsonPathBySource = getAbsolutePath(
      `${page}.${config.fileSource}.json`,
    );
    const pageJsonPathByPlatform = getAbsolutePath(
      `${page}.${config.platform}.json`,
    );

    if (fs.existsSync(pageJsonPathBySource)) {
      entryConfigPath.add(pageJsonPathBySource);
      entry[entryName].push(pageJsonPathBySource);
    } else if (fs.existsSync(pageJsonPathByPlatform)) {
...
function parseComponents(componentsUserPath, components) {
  ...
  const componentPathList = Object.values(components)
    ...
    .map((pathUrl) => {
      ...
      const configPathBySource = getAbsolutePath(
        `${pathUrl}.${config.fileSource}.json`,
      );
      const configPathByPlatform = getAbsolutePath(
        `${pathUrl}.${config.platform}.json`,
      );

      if (fs.existsSync(configPathBySource)) {
        entryConfigPath.add(configPathBySource);
      } else if (fs.existsSync(configPathByPlatform)) {
