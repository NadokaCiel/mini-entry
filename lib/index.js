 ...
 * @Last Modified time: 2021-05-12 11:16:51
...
function getAppEntry(configFile) {
  ...
  const scriptPath = join(
    process.cwd(),
    configFile.replace(extname(configFile), `.${config.entrySuffix.js}`),
  );
  ...
  if (fs.existsSync(scriptPath)) {
    return {
      [`${entryName}`]: [absolutePath, scriptPath],
    };
  }
...
function parsePages(pages) {
  ...
  pages.forEach((page) => {
    ...
    if (config.entrySuffix.css) {
      const stylePath = getAbsolutePath(`${page}.${config.entrySuffix.css}`);
